...
impl Data {
    fn new(version: i32, data_type: i32, ecc_level: i32, eci: u32, mask: i32, payload: String) -> Data {
        Data {
            version,
            data_type,
            ecc_level,
...
unsafe fn validate_against_original(path: &Path, expected_contents: &[Option<Data>]) {
    ...
    let result: Vec<_> = (0..quirc_count(decoder as (*const quirc))).map(|i| {
...
check!(test_image_6, "Moe_Epsilon_QR_code.png", &[Some(Data { version: 4, data_type: 4, ecc_level: 0, eci: 0, mask: 1, payload: "http://en.wikipedia.org/wiki/User:Moe_Epsilon".to_owned() })]);
check!(test_image_7, "QR_CC_Chief_Taza.JPG", &[]);
check!(test_image_8, "QRCode-1-Intro.png", &[Some(Data { version: 3, data_type: 4, ecc_level: 1, eci: 0, mask: 7, payload: "Mr. Watson, come here - I want to see you.".to_owned() })]);
check!(test_image_9, "QRCode-2-Structure.png", &[]);
check!(test_image_10, "QR_code_Congressional_Cemetery.jpg", &[Some(Data { version: 3, data_type: 4, ecc_level: 1, eci: 0, mask: 3, payload: "http://en.qrwp.org/Congressional_Cemetery".to_owned() })]);
check!(test_image_11, "QR_Code_Damaged.jpg", &[Some(Data { version: 3, data_type: 4, ecc_level: 3, eci: 0, mask: 1, payload: "http://en.m.wikipedia.org".to_owned() })]);
check!(test_image_12, "Qr_code_details.png", &[None]);
check!(test_image_13, "QRcode_-_De_Verdieping_van_Nederland_(3).jpg", &[Some(Data { version: 4, data_type: 4, ecc_level: 1, eci: 0, mask: 3, payload: "http://nl.wikipedia.org/wiki/Geschiedenis_van_de_Nederlandse_slavernij".to_owned() })]);
check!(test_image_14, "QRcode_-_De_Verdieping_van_Nederland_(4).JPG", &[Some(Data { version: 4, data_type: 4, ecc_level: 1, eci: 0, mask: 3, payload: "http://nl.wikipedia.org/wiki/Geschiedenis_van_de_Nederlandse_slavernij".to_owned() })]);
check!(test_image_15, "QR_code_for_QRpedia.png", &[Some(Data { version: 2, data_type: 4, ecc_level: 1, eci: 0, mask: 4, payload: "http://en.qrwp.org/QRpedia".to_owned() })]);
check!(test_image_16, "QR_Code,_Museum_für_Hamburgische_Geschichte_IMG_1607_original.jpg", &[None]);
check!(test_image_17, "QR-code-Open-research.png", &[Some(Data { version: 3, data_type: 4, ecc_level: 1, eci: 0, mask: 2, payload: "http://en.qrwp.org/Open_research".to_owned() })]);
...
check!(test_image_20, "Qrcode_wikipedia_fr_v2clean.png", &[Some(Data { version: 2, data_type: 4, ecc_level: 1, eci: 0, mask: 1, payload: "http://fr.wikipedia.org/".to_owned() })]);
...
check!(test_image_23, "Sk.wikipedia.org_QR_Code.png", &[Some(Data { version: 2, data_type: 4, ecc_level: 1, eci: 0, mask: 2, payload: "http://sk.wikipedia.org/".to_owned() })]);
