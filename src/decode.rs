...
unsafe extern fn grid_bit(
    mut code : *const quirc_code, mut x : i32, mut y : i32
) -> i32 {
...
unsafe extern fn format_syndromes(
    mut u : u16, mut s : *mut u8
) -> i32 {
    let mut i : i32;
...
unsafe extern fn berlekamp_massey(
    mut s : *const u8,
    mut N : i32,
    mut gf : *const galois_field,
    mut sigma : *mut u8
...
unsafe extern fn poly_eval(
    mut s : *const u8, mut x : u8, mut gf : *const galois_field
) -> u8 {
    let mut i : i32;
    let mut sum : u8 = 0u8;
...
unsafe extern fn read_format(
    ...
    if which != 0 {
        ...
        'loop6: loop {
            ...
            format = (format as (i32) << 1i32 | grid_bit(
                                                    code,
                                                    8i32,
                                                    (*code).size - 1i32 - i
                                                )) as (u16);
...
unsafe extern fn reserved_cell(
    mut version : i32, mut i : i32, mut j : i32
) -> i32 {
    let mut ver
...
unsafe extern fn mask_bit(
    mut mask : i32, mut i : i32, mut j : i32
) -> i32 {
    if mask == 7i32 {
...
unsafe extern fn block_syndromes(
    mut data : *const u8, mut bs : i32, mut npar : i32, mut s : *mut u8
) -> i32 {
    let mut nonzero : i32 = 0i32;
    let mut i : i32;
    memset(s as (*mut ::std::os::raw::c_void),0i32,64usize);
    ...
    'loop1: loop {
        ...
        'loop4: loop {
            ...
            if !(c == 0) {
                let _rhs
                    = gf256_exp[
                          ((gf256_log[c as (usize)] as (i32) + i * j) % 255i32) as (usize)
                      ];
                let _lhs = &mut *s.offset(i as (isize));
                *_lhs = (*_lhs as (i32) ^ _rhs as (i32)) as (u8);
            }
        ...
        if !(a == 0) {
            ...
            'loop5: loop {
                ...
                if !(b == 0) {
                    let _rhs
                        = gf256_exp[
                              ((log_a as (i32) + gf256_log[
                                                     b as (usize)
                                                 ] as (i32)) % 255i32) as (usize)
                          ];
...
unsafe extern fn correct_block(
        ...
        'loop4: loop {
            ...
            if poly_eval(
                ...
                let mut error
                    : u8
                    = gf256_exp[
                          ((255i32 - gf256_log[sd_x as (usize)] as (i32) + gf256_log[
                                                                               omega_x as (usize)
                                                                           ] as (i32)) % 255i32) as (usize)
                      ];
                let _rhs = error;
...
unsafe extern fn bits_remaining(
    mut ds : *const datastream
) -> i32 {
    (*ds).data_bits - (*ds).ptr
}

unsafe extern fn take_bits(
    mut ds : *mut datastream, mut len : i32
) -> i32 {
...
unsafe extern fn numeric_tuple(
    mut data : *mut quirc_data,
    mut ds : *mut datastream,
    mut bits : i32,
    mut digits : i32
...
unsafe extern fn decode_numeric(
    mut data : *mut quirc_data, mut ds : *mut datastream
) -> Enum1 {
    let mut _currentBlock;
...
unsafe extern fn alpha_tuple(
    ...
    if bits_remaining(ds as (*const datastream)) < bits {
        ...
        'loop2: loop {
            ...
            static mut alpha_map
                : *const u8
                = (*b"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./:\0").as_ptr();
            (*data).payload[
                ((*data).payload_len + digits - i - 1i32) as (usize)
...
unsafe extern fn decode_byte(
    ...
    if (*data).payload_len + count + 1i32 > 8896i32 {
        Enum1::QUIRC_ERROR_DATA_OVERFLOW
    } else if bits_remaining(
                  ds as (*const datastream)
              ) < count * 8i32 {
        Enum1::QUIRC_ERROR_DATA_UNDERFLOW
    } else {
        i = 0i32;
...
unsafe extern fn decode_payload(
    mut data : *mut quirc_data, mut ds : *mut datastream
) -> Enum1 {
    let mut _currentBlock;
...
#[no_mangle]
pub unsafe extern fn quirc_decode(
